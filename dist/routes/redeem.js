 ...
 */
router.get("/redeem/shipping-info", auth_1.auth, async (req, res) => {
    try {
        const { userId } = req;
 ...
 */
router.post("/redeem/shipping-info", auth_1.auth, async (req, res) => {
    try {
        const { userId } = req;
 ...
 */
router.post("/redeem/claim-physical", auth_1.auth, async (req, res) => {
    try {
        const { userId } = req;
        ...
        const redeemedCar = await prisma_1.prisma.car.update({
            where: { tokenId: parseInt(tokenId) },
            data: {
                isRedeemed: true,
                redeemedAt: new Date(),
