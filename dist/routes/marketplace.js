...
const marketplace_client_1 = require("../blockchain/marketplace-client");
 ...
 */
router.post("/marketplace/list", auth_1.auth, async (req, res) => {
    try {
        ...
        res.status(201).json({
            ...
            nextSteps: "IMPORTANT: You must approve the backend wallet to transfer your NFT. " +
 ...
 */
router.get("/marketplace/listings", auth_1.auth, async (req, res) => {
    try {
 ...
 */
router.post("/marketplace/buy/:listingId", auth_1.auth, async (req, res) => {
    try {
        const { userId: buyerUserId, walletAddress: buyerWallet } = req;
        ...
        let listing;
        try {
            listing = await prisma_1.prisma.$transaction(async (tx) => {
        ...
        try {
            const balance = await (0, client_1.getMockIDRXBalance)(buyerWallet);
        ...
        const hasAllowance = await (0, marketplace_client_1.verifyIDRXAllowance)(buyerWallet, listing.price);
        if (!hasAllowance) {
            // Rollback to active
            await prisma_1.prisma.listing.update({
                where: { id: listingId },
        ...
        const sellerOwns = await (0, marketplace_client_1.verifyCarOwnership)(listing.carTokenId, listing.seller.walletAddress);
        if (!sellerOwns) {
        ...
        const isApproved = await (0, marketplace_client_1.verifyCarApproval)(listing.carTokenId);
        if (!isApproved) {
            // Rollback to active
            await prisma_1.prisma.listing.update({
                where: { id: listingId },
        ...
        let txHash;
        try {
            const result = await (0, marketplace_client_1.executePurchase)(listing.carTokenId, buyerWallet, listing.seller.walletAddress, listing.price);
 ...
 */
router.delete("/marketplace/cancel/:listingId", auth_1.auth, async (req, res) => {
    try {
        const { userId } = req;
        const listingId = parseInt(req.params.listingId);
        if (isNaN(listingId)) {
 ...
 */
router.get("/marketplace/my-listings", auth_1.auth, async (req, res) => {
    try {
        const { userId } = req;
 ...
 */
router.get("/marketplace/listing/:listingId", auth_1.auth, async (req, res) => {
    try {
        const listingId = parseInt(req.params.listingId);
        if (isNaN(listingId)) {
            res.status(400).json({ error: "Invalid listing ID" });
